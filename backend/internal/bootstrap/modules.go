...
// "故障修复/关闭后办结在办派工单" 需要故障模块反向通知派工模块,
// 因此通过 SetOpenFaultCounter / SetResolvedListener 在构造完成后回填, 避免构造循环依赖。
func buildModules(db *gorm.DB) []module.Module {
	...
	dispatchModule := dispatch.New(db, faultModule.Service())
	faultModule.Service().SetResolvedListener(dispatchModule.Service())
